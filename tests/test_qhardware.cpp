...
TEST(HardwareSpecificationTest, MeasurementChannel) {
    HardwareSpecification spec(Algiers, false, true);
    ...
    Instruction meas_instruction(GateName::Meas, 1, 0);
    spec.instructions_to_channels[make_shared<Instruction>(meas_instruction)]->print_channel();

    Instruction meas_instruction2(GateName::Meas, 0, 0);
    hs.instructions_to_channels[make_shared<Instruction>(meas_instruction2)]->print_channel();
