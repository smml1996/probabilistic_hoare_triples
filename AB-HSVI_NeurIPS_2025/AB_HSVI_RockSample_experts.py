...
def AB_HSVI(model,disc,epsilon,results_file):
    ...
    return gamma,S,A,O,transitions,observations,rewards,initial_beliefs


def belief_update_local(bel, a, o, e_transition, e_observation):
    new_values = sp.sparse.dok_matrix((1,S),dtype=float)
    new_bel = 0
    valid_bel = True

    trns = e_transition[a][0]
    obs = e_observation[a][0]
    denom = sum([bel.values[0,s]*trns[s,ss]*obs[ss,o] for s in range(S) for ss in range(S)])
    if denom > 0:
        for ss in range(S):
            new_values[0,ss] = sum([bel.values[0,s]*trns[s,ss]*obs[ss,o] for s in range(S)])/denom
        new_bel = Belief(new_values.tocsr())
    else:
        valid_bel = False
...
def comp_e_i(e,i,S,A,O,e_gammas,e_transitions,e_observations,e_rewards,e_belief,i_belief,disc,n):
        if n > 0:
            vals = [np.sum(alpha.env_values.multiply(e_belief.values)) for alpha in e_gammas[e]]
            action = e_gammas[e][np.argmax(vals)].action
            value = np.sum([e_rewards[i][0][s,action]*i_belief.values[0,s] for s in range(S)])
            for o in range(O):
                p_e_o = 0
                p_i_o = 0
                e_trns = e_transitions[e][action][0]
                e_obs = e_observations[e][action][0]
                i_trns = e_transitions[i][action][0]
                i_obs = e_observations[i][action][0]
                for s in range(S):
                    e_bel_ns = e_belief.values[0,s]
                    i_bel_ns = i_belief.values[0,s]
                    for ss in range(S):
                        p_e_o += e_bel_ns*e_trns[s,ss]*e_obs[ss,o]
                        p_i_o += i_bel_ns*i_trns[s,ss]*i_obs[ss,o]
                if (p_i_o > 0) and (p_e_o > 0):
                    new_e_bel, _ = belief_update_local(e_belief,action,o,e_transitions[e],e_observations[e])
                    new_i_bel, _ = belief_update_local(i_belief,action,o,e_transitions[i],e_observations[i])
                    value += disc*p_i_o*comp_e_i(e,i,S,A,O,e_gammas,e_transitions,e_observations,e_rewards,new_e_bel,new_i_bel,disc,n-1)
                elif (p_i_o > 0) and (p_e_o == 0):
                    print("Error, unexpected observation") #This cannot happen in the RockSample experts, since they have no reason to check the states of the rocks.
            return value
        return 0

def experts_comp(n,g,k,r):
    e_gammas = []
    S,A,O = 0,0,0
    e_transitions = []
    e_observations = []
    e_rewards = []
    initial_beliefs = sp.sparse.dok_matrix((1,S),dtype=float)

    experts = sp.special.comb(k,g,exact=True)
    for e in range(experts):
        test = f"RockSample_MEPOMDP_N{n}_G{g}_K{k}_R{r}_expert{e}"
        print(f"\nStarting test {test}:\n")
        gamma,S,A,O,transitions,observations,rewards,initial_beliefs = AB_HSVI.AB_HSVI(f"Models/{test}.txt",0.95,1,f"Results/{test}.txt")
        e_gammas.append(gamma)
        e_transitions.append(transitions)
        e_observations.append(observations)
    ...
    for e in range(experts):
        for i in range(experts):
            if e != i:
                value = comp_e_i(e,i,S,A,O,e_gammas,e_transitions,e_observations,e_rewards,Belief(initial_beliefs),Belief(initial_beliefs),0.95,45)
                print(f"Value of using policy of expert {e} in environment {i}: {value:.6f}")
                print_buffer.append(f"Value of using policy of expert {e} in environment {i}: {value:.6f}")

    results_file = f"Results/RockSample_MEPOMDP_N{n}_G{g}_K{k}_R{r}_expert_summary.txt"
    with open(results_file, "w") as file:
        file.write("\n".join(print_buffer))

# Run the AB-HSVI algorithm on the ME-POMDP RockSample problem with an n x n grid, g good rocks, and k rocks in total, using random seed r to create the rock positions,
# and on each POMDP in this ME-POMDP. Compute the value each POMDP agent policy achieves on the other environments (other POMDPs in the ME-POMDP).
if __name__ == "__main__":
    ...
    for e in range(experts):
        test = f"RockSample_MEPOMDP_N{n}_G{g}_K{k}_R{r}_expert{e}"
        print(f"\nStarting test {test}:\n")
        gamma,S,A,O,transitions,observations,rewards,initial_beliefs = AB_HSVI(f"Models/{test}.txt",0.95,1,f"Results/{test}.txt")
        e_gammas.append(gamma)
        e_transitions.append(transitions)
        e_observations.append(observations)
        e_rewards.append(rewards)
    ...
    for e in range(experts):
        for i in range(experts):
            if e != i:
                value = comp_e_i(e,i,S,A,O,e_gammas,e_transitions,e_observations,e_rewards,Belief(initial_beliefs),Belief(initial_beliefs),0.95,45)
                print(f"Value of using policy of expert {e} in environment {i}: {value:.2f}")